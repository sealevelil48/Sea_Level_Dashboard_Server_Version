...
class SeaLevelDashboard:
    ...
    <body>
        ...
        <script>
            govmap.token = "11aa3021-4ae0-4771-8ae0-df75e73fe73e"; // Replace with a valid token!

            govmap.createMap('map-container', {
                layers: [],
                center: { x: 176505, y: 662250 },
                zoom: 0, // Adjusted for better visibility of Israel
                basemap: '2',
                isPopupOpen: false
            });
            ...
            fetch('/api/stations/map')
                ...
                .then(stations => {
                    ...
                    // Calculate min and max sea level for scaling
                    const seaLevels = stations.map(s => s.latest_value).filter(v => v !== null && !isNaN(v));
                    const seaLevelMin = seaLevels.length > 0 ? Math.min(...seaLevels) : NaN;
                    const seaLevelMax = seaLevels.length > 0 ? Math.max(...seaLevels) : NaN;
                    const minRadius = 50; // meters
                    const maxRadius = 200; // meters
                    const defaultRadius = 100; // meters

                    const circleGeometries = stations.map(s => {
                        const val = s.latest_value;
                        let radius = defaultRadius;
                        if (seaLevels.length > 0 && !isNaN(seaLevelMin) && !isNaN(seaLevelMax)) {
                            if (seaLevelMax !== seaLevelMin) {
                                if (val !== null && !isNaN(val)) {
                                    radius = minRadius + ((val - seaLevelMin) / (seaLevelMax - seaLevelMin)) * (maxRadius - minRadius);
                        ...
                        return {x: s.x, y: s.y, radius: radius};
                    });

                    const names = stations.map(s => s.Station || "Unknown");
                    const tooltips = stations.map(s => {
                        const val = s.latest_value !== null ? Number(s.latest_value).toFixed(3) : "N/A";
                        const date = s.last_update || "N/A";
                        return `Station: ${s.Station}\\nSea Level: ${val} m\\nLast Update: ${date}`;
                    });
                    const bubbleTemplate = `<div><strong>Station:</strong> {0}<br/><strong>Sea Level:</strong> {1} m<br/><strong>Last Update:</strong> {2}</div>`;
                    const bubbleHTMLParameters = stations.map(s => {
                        const val = s.latest_value !== null ? Number(s.latest_value).toFixed(3) : "N/A";
                        const date = s.last_update || "N/A";
                        return [s.Station, val, date];
                    });

                    govmap.displayGeometries({
                        circleGeometries: circleGeometries,
                        names: names,
                        geometryType: govmap.geometryType.CIRCLE,
                        clearExisting: true,
                        data: {
                            tooltips: tooltips,
                            bubbleHTML: bubbleTemplate,
                            bubbleHTMLParameters: bubbleHTMLParameters
                        },
                        defaultSymbol: {
                            outlineColor: [255, 0, 0, 1], // Red outline
                            outlineWidth: 2,
                            fillColor: [255, 0, 0, 0.3] // Semi-transparent red fill
                        }
                    });
                })
                .catch(err => {
    ...
    def register_optimized_callbacks(self, app):
        ...
        # Core data updates for graph, table data store, and stats - FIXED
        @app.callback(
        ...
        # Update map view - FIXED to prevent infinite loops
        @app.callback(
